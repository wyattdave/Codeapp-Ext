...
import { determineTypeScriptType, extractModelName } from './shared/modelUtils.js';
...
async function handleSwaggerSchema(context) {
    ...
    const definitionTypes = processDefinitions(swaggerObject, modelLines);
...
function generateModelFromSchema(modelName, definition) {
    ...
    if (isArrayType) {
        ...
        // For array types with inline item schemas, generate an interface for the item type and a type alias for the array
...
function analyzeSchemaStructure(definition) {
    ...
    return { isEmpty: !hasProperties && !hasItems, itemsNode };
}
/**
...
function processRequestParameters(param, modelName, imports, parametersList, parametersProps, paramNodeName, parameterTypes, originalParamEntries, entityModelMap) {
    ...
    if (parameterTypes && parameterTypes[paramName]) {
        ...
        originalParamEntries?.push(`${parameterTypes[paramName].propertyDefinition}`);
    }
    else {
        ...
        // Normalize parameter name (hyphens/dots → underscores for valid TS identifier)
